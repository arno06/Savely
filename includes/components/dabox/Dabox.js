...
#Dabox{left:50%;margin-top:40px;background:#ffffff url(../imgs/loader.gif) no-repeat center center;}
#Dabox .M4Close {display:block;position:absolute;top:10px;right:10px;height:0;width:34px;padding-top:34px;overflow:hidden;background: #000;}
 */
...
var Dabox = (function()
{
	...
	function initDaBox()
	{
		document.querySelectorAll('*[rel^="Dabox"]').forEach(function(a)
		{
			...
			t = r.match(/Dabox\[([a-z0-9\_\-]+)\]/)[1];
			if(t&&document.getElementById(t))
			{
				publicAPI.register(a, t);
			}
			t = false;
			r.sub(/Dabox\[ajax\:([^\]]+)\]/, function (match){return t=match[1];});
			if(t)
			{
				a.rel = t;
				a.addEventListener('click', aClickAjaxBox);
			}
		});
	}
